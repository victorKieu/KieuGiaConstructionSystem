#!/bin/bash
# Script để sửa tất cả các trang sử dụng Supabase

echo "Tìm tất cả các trang sử dụng Supabase..."
PAGES=$(find app -type f -name "*.tsx" -o -name "*.ts" | grep -v "api" | xargs grep -l "supabase")

echo "Cập nhật các trang..."
for page in $PAGES; do
  ...
  # Thêm kiểm tra isSupabaseReady() vào đầu các hàm không async
  sed -i '/export default function/a\  // Kiểm tra xem Supabase có sẵn sàng không\n  if (typeof window === "undefined" \&\& !isSupabaseReady()) {\n    return (\n      <div className="container mx-auto p-4">\n        <div className="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4" role="alert">\n          <p className="font-bold">Cảnh báo</p>\n          <p>Không thể kết nối đến Supabase. Vui lòng kiểm tra biến môi trường.</p>\n        </div>\n      </div>\n    )\n  }' "$page"
done
